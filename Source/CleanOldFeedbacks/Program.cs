...
namespace CleanOldFeedbacks
{
    internal class Program
    {
        ...
        private static readonly List< string > ExceptList = new List< string > { "3.1.0", "Bugs" };
        ...
        private static IEnumerable< string > GetBadFiles( string path )
        {
            ...
            var badIPs = new[] { "62.141.68.238", "62.141.68.236", "195.151.213.241", "194.84.231.43" };
            var badComputerNames = new[] { "DONNA-PC" };
            return groupsFiles.SelectMany( gr =>
                                               {
                                                   var badFiles =
                                                       gr.Where( fl => fl.fileInfo.Item3 == DummyDate || badIPs.Any( ip => fl.fileInfo.Item1 == ip ) || badComputerNames.Any( comp => fl.fileInfo.Item2 == comp ) ).ToList();
